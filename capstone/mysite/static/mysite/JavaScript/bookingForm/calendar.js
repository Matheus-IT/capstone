document.addEventListener('DOMContentLoaded', function() {
	const calendar = {
		...
		handleGoToNextMonth(event) {
			...
			const dateNextMonth = new Date(this.date.getFullYear(), this.nextMonth-1);
			this.displayMonth.innerHTML = dateNextMonth.toLocaleString('default', { month: 'long' });
	
			const daysInThisMonth = new Date(this.date.getFullYear(), this.nextMonth, 0).getDate();
		...
		handleGoToCurrentMonth(event) {
			...
			this.displayMonth.innerHTML = this.date.toLocaleString('default', { month: 'long' });
			this.displayYear.innerHTML = this.date.getFullYear();
			
			const daysInThisMonth = new Date(this.date.getFullYear(), this.currentMonth, 0).getDate();
			this.generateDays(daysInThisMonth);
		...
		async generateDays(daysInThisMonth) {
			...
			for (let i = 1; i <= daysInThisMonth; i++) {
				...
				newButton.addEventListener('click', (event) => this.handleSelectDay(event));
