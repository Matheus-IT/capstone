...
class QueueConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        try:
            received_group_name = self.scope['url_route']['kwargs'][
                'group_name'
            ]
            user_was_added = await self.handle_add_user_to_channel_group(
                received_group_name
            )
            ...
            print(queue)
            await self.send(
                text_data=json.dumps({'queueSize': queue.num_of_people})
            )
